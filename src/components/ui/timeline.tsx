    ...
    >
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        ...
        <p className="text-muted-foreground text-lg md:text-xl max-w-2xl">
          Tracing the path from early explorations to specialized deep dives in 3D Computer Vision and Dance integration.
        </p>
      </div>
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-10 absolute left-3 md:left-3 w-10 border border-border rounded-full bg-background flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-foreground border border-neutral-300 p-2" />
              </div>
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              ...
              <h4 className="text-2xl md:text-4xl font-bold text-foreground mb-6">
                {item.heading}
              </h4>
              ...
              <div className="flex flex-wrap gap-2">
                 {item.tags?.map((tag) => (
                    <span key={tag} className="px-4 py-2 bg-muted/50 border border-border rounded-full text-sm text-foreground font-semibold tracking-wide">
                      {tag}
                    </span>
                 ))}
              </div>
